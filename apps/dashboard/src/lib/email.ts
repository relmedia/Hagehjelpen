...
// this keeps the dashboard free of a rendering dependency.
    ...
    .map(
      (row) => `
        <tr>
          ...
          <td style="padding:6px 0;color:${COLORS.ink};font-size:14px;font-weight:600;">${escapeHtml(row.value)}</td>
        </tr>`,
    )
    .join("");
...
export function renderEmail(options: EmailOptions): string {
  const badge = options.badge
    ? BADGE_TONES[options.badge.tone ?? "info"]
    : null;

  const paragraphs = (options.intro ?? [])
    .map(
      (text) =>
        `<p style="margin:0 0 14px;font-size:15px;line-height:1.7;color:${COLORS.inkSoft};">${escapeHtml(text)}</p>`,
    )
    .join("");
